...
namespace PlatformClient.Utility
{
    ...
    /// 生成验证码类
    /// </summary>
    class AuthenticodeHelper
    {
        Random r = new Random(DateTime.Now.Millisecond);

        internal string CreateIndentifyCode(int count)
        {
        ...
        internal void CreatIndentifyCodeImage(string Text, Image imgsource, int iWidth, int iHight)
        {
            Grid Gx = new Grid();
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                {
                    p.Points.Add(new Point(r.NextDouble() * iWidth, r.NextDouble() * iHight));
                }
            ...
            int y = 0;
            int lw = 6;
            double w = (iWidth - lw) / Text.Length;
            int h = (int)iHight;
